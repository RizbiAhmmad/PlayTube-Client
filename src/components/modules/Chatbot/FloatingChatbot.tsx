...
const INITIAL_MESSAGES: Message[] = [
  {
    ...
    role: "bot",
    content:
      "Hello! I'm your PlayTube AI assistant 👋\n\nAsk me anything about our movies and shows — titles, genres, actors, or what other viewers think. I'll find the best recommendation for you.",
  },
];

const SUGGESTED_QUERIES = [
  "Best action movies available?",
  "Suggest some Sci-Fi thrillers",
...
export default function FloatingChatbot() {
  ...
  const handleSync = () => {
    startSyncTransition(async () => {
      const result = await ingestMediaAction();
      if (result.success) {
        toast.success(`Media data synced!`, {
          description:
            result.message ?? `${result.indexedCount ?? 0} items indexed.`,
        });
      } else {
        toast.error("Sync failed", { description: "Failed to sync media data." });
      }
    });
  };
        ...
        <div className="shrink-0 px-3 py-3 bg-white border-t border-slate-100">
          ...
          >
            <input
              ref={inputRef}
              type="text"
              placeholder="Ask about movies, genres..."
              value={inputValue}
