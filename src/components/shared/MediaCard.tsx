...
const MediaCard = ({ media }: { media: IMedia }) => {
  return (
    <Link href={`/media/${media.id}`} className="group relative block overflow-hidden rounded-xl bg-card border transition-all hover:border-primary/50 hover:shadow-xl hover:shadow-primary/10">
      {/* Poster Image */}
      <div className="relative aspect-[2/3] w-full overflow-hidden">
        <Image
          src={media.thumbnail || "https://unsplash.com/photos/a-film-clapper-and-some-reels-on-a-table-mQySTK6ejR8"}
          alt={media.title}
          fill
        ...
        <div className="absolute left-3 top-3 flex flex-col gap-2">
          ...
          {media.pricingType === "PREMIUM" && (
            <Badge variant="secondary" className="bg-amber-500/90 text-white border-none text-[10px] font-bold uppercase tracking-wider backdrop-blur-sm">
              Premium
            </Badge>
          )}
        </div>
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center justify-between text-sm text-muted-foreground font-medium">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-foreground">{media.averageRating || "0.0"}</span>
            <span className="text-[10px] text-muted-foreground">({media.reviewCount || 0})</span>
          </div>
          <div className="flex items-center gap-1.5">
